...
def avvia_elezione(nodo):
    ...
    superiori = [peer for peer in nodo.lista_nodi if peer > nodo.id]
    if not superiori:
...
def gestisci_risposta_ok(nodo, mittente_id):
    print(f"[Nodo {nodo.id}] Ricevuto OK da {mittente_id}")
...
def verifica_risposte(nodo):
    if not nodo.risposte_ok:
        ...
        print(f"[Nodo {nodo.id}] Attendo coordinatore...")

# Ora il nodo da normale diventa leader
def proclama_leader(nodo):
    nodo.stato = "leader"
    nodo.leader = nodo.id
    print(f"[Nodo {nodo.id}] Mi proclamo leader!")
    for peer_id in nodo.lista_nodi:
        if peer_id != nodo.id:
...
def ricevi_coordinatore(nodo, nuovo_leader_id):
    ...
    print(f"[Nodo {nodo.id}] Riconosco {nuovo_leader_id} come leader.")