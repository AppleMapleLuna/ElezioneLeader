...
TIMEOUT = 10  #secondi di attesa per le risposte OK
...
def avvia_elezione(nodo):
    ...
    if not superiori:
        threading.Timer(TIMEOUT, verifica_risposte, args=(nodo,)).start()
        return
...
def verifica_risposte(nodo):
    if nodo.leader is not None:
        print(f"[Nodo {nodo.id}] ha già un leader ({nodo.leader}), non si proclama.")
        nodo.stato = "normale"
        return
...
def verifica_coordinatore(nodo):
    if not nodo.coordinatore_ricevuto:
        print(f"[Nodo {nodo.id}] coordinatore non ricevuto, avvio nuova elezione.")
        nodo.start_election()
    else:
        print(f"[Nodo {nodo.id}] ha già riconosciuto il leader {nodo.leader}, nessuna nuova elezione.")

def proclama_leader(nodo):
    if nodo.stato == "leader" or nodo.leader is not None or nodo.risposte_ok:
        print(f"[Nodo {nodo.id}] non può proclamarsi leader (leader già noto o OK ricevuto).")
        return
    nodo.stato = "leader"
    nodo.leader = nodo.id
    nodo.coordinatore_ricevuto = True
    try:
        requests.post(f"http://{nodo.server_ip}:8000/set_leader", data={"id": nodo.id})
    except:
        print(f"[Nodo {nodo.id}] errore nel comunicare il leader al server.")
